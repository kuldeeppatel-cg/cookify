...
const MyRecipes = () => {
  ...
  const { 
    ...
    user, toggleSaved, toggleFavorite, 
    savedRecipes, favoriteRecipes, 
    addRecent, deleteRecipe 
  } = useRecipeContext();
  ...
  const myRecipes = useMemo(() => {
    ...
    return recipes.filter(r => r && r.createdBy === user);
  }, [recipes, user]);
  ...
  if (loading || isUserLoading) {
    return (
      <div className="min-h-screen pt-32 pb-12 px-6 flex flex-col items-center justify-center bg-bg-primary">
        ...
        <h2 className="text-xl md:text-2xl text-text-secondary font-medium animate-pulse mt-2">Gathering your culinary masterpieces...</h2>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen pt-32 px-6 flex flex-col items-center justify-center text-center">
        <div className="bg-bg-secondary p-10 rounded-3xl border border-red-500/20 max-w-md">
          ...
          <p className="text-text-secondary mb-8">We couldn't load your recipes. Please check your connection and try again.</p>
          <button 
